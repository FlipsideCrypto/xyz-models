{% macro create_aws_aurora_api() %}
    {% if target.name == "prod" %}
        {% set sql %}
        CREATE api integration IF NOT EXISTS aws_bitcoin_api api_provider = aws_api_gateway api_aws_role_arn = 'arn:aws:iam::490041342817:role/bitcoin-api-prod-rolesnowflakeudfsAF733095-YHUYTRLX42SE' api_allowed_prefixes = (
            'https://9iyki2rp1b.execute-api.us-east-1.amazonaws.com/prod/'
        ) enabled = TRUE;
        {% endset %}
        {% do run_query(sql) %}
    {% else %}
