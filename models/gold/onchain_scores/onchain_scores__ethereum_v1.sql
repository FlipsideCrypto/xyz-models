...
WITH cex_addresses AS (
    ...
    FROM {{ source('ethereum_gold_core', 'dim_labels') }}
...
activity AS (
    ...
    FROM {{ source('ethereum_gold_core', 'fact_transactions') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    AND from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
...
from_bridge_daily AS (
    ...
    FROM {{ source('ethereum_gold_defi','ez_bridge_activity') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND receiver NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    AND receiver NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
...
from_cex_daily AS (
    ...
    FROM {{ source('ethereum_gold_core', 'ez_native_transfers') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
    ...
    SELECT 
        ...
        DATE(block_timestamp) AS activity_day,
        COUNT(*) AS n_cex_withdrawals
    FROM {{ source('ethereum_gold_core','ez_token_transfers')}} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND FROM_ADDRESS IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
...
nft_buys AS (
    ...
    FROM {{ source('ethereum_gold_nft','ez_nft_sales') }} 
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND buyer_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    AND buyer_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
...
from {{ source('ethereum_gold_core','ez_decoded_event_logs') }} 
where topics[0] in (
...
FROM rawmints
WHERE
decoded_log:to NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
AND 
decoded_log:to NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
group by user_address
),
-- governance (just liquid staking on ethereum)
gov_mints AS (
    SELECT
        to_address AS user_address,
    ...
    FROM {{ source('ethereum_gold_core','ez_token_transfers')}}
    WHERE 
        contract_address IN (select distinct lower(token_address) from ethereum.defi.ez_liquid_staking_deposits)
...
gov_restakes AS (
    ...
    FROM {{ source('ethereum_gold_core','ez_decoded_event_logs') }}
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address IN (select distinct lower(token_address) from ethereum.defi.ez_liquid_staking_deposits)
    AND event_name IN ('Deposit', 'Submitted', 'DepositedFromStaking')
    AND origin_from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
    AND origin_from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
...
swaps_in AS (
    SELECT 
        origin_from_address AS user_address, 
        COUNT(*) AS n_swap_tx
    FROM {{ source('ethereum_gold_defi','ez_dex_swaps')}}
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND origin_from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
    AND origin_from_address NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    GROUP BY origin_from_address
...
lp_adds AS (
    SELECT 
        ...
        COUNT(*) AS n_lp_adds
    FROM {{ source('ethereum_gold_core','ez_token_transfers')}}
    WHERE TO_ADDRESS IN (SELECT contract_address FROM {{ source('ethereum_gold_defi','ez_dex_swaps')}} WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND TX_HASH NOT IN (SELECT TX_HASH FROM {{ source('ethereum_gold_defi','ez_dex_swaps')}} WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_labels') }})
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('ethereum_gold_core', 'dim_contracts') }})
    GROUP BY FROM_ADDRESS
),
...
tx_hash
FROM
{{ source('ethereum_gold_core','ez_token_transfers')}}
WHERE
to_address IN (SELECT pool_address FROM ethereum.defi.dim_dex_liquidity_pools)
AND
block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
GROUP BY tx_hash
),
...
other_defi AS (
    ...
    FROM {{ source('ethereum_gold_core','ez_decoded_event_logs') }}
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address NOT IN (select distinct lower(token_address) from ethereum.defi.ez_liquid_staking_deposits)
...
scores AS (
    SELECT
        user_address,
        (CASE WHEN n_days_active > 6 THEN 1 ELSE 0 END
         + CASE WHEN n_complex_txn > 4 THEN 1 ELSE 0 END
        ...
        (CASE WHEN n_other_defi > 1 THEN 1 ELSE 0 END
         + CASE WHEN n_swap_tx > 1 THEN 1 ELSE 0 END
