{{ config (
    ...
    full_refresh = false,
    tags = ['gold', 'onchain_scores', 'optimism_scores'],
    version = 1
) }}
...
WITH cex_addresses AS (
    ...
    FROM {{ source('optimism_gold_core','dim_labels')}}
...
activity AS (
    ...
    FROM {{ source('optimism_gold_core','fact_transactions') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    AND from_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
...
from_bridge_daily AS (
    ...
    FROM {{ source('optimism_gold_defi','ez_bridge_activity') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND receiver NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    AND receiver NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
...
from_cex_daily AS (
    ...
    FROM {{ source('optimism_gold_core','ez_native_transfers')}} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
    ...
    SELECT 
        ...
        DATE(block_timestamp) AS activity_day,
        COUNT(*) AS n_cex_withdrawals
    FROM {{ source('optimism_gold_core','ez_token_transfers') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND FROM_ADDRESS IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
...
nft_buys AS (
    ...
    FROM {{ source('optimism_gold_nft','ez_nft_sales') }} 
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND buyer_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    AND buyer_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
...
from {{ source('optimism_gold_core','ez_decoded_event_logs') }} 
where topics[0] in (
...
FROM rawmints
WHERE
decoded_log:to NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
AND 
decoded_log:to NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
group by user_address
),
-- governance (just liquid staking on optimism)
gov_mints AS (
    SELECT
        to_address AS user_address,
    ...
    FROM {{ source('optimism_gold_core','ez_token_transfers') }}
    WHERE 
        contract_address IN (lower('0x6806411765Af15Bddd26f8f544A34cC40cb9838B'),
                     lower('0xadDb6A0412DE1BA0F936DCaeb8Aaa24578dcF3B2'),
                     lower('0xE405de8F52ba7559f9df3C368500B6E6ae6Cee49'),
                     lower('0xc45A479877e1e9Dfe9FcD4056c699575a1045dAA'),
                     lower('0x1F32b1c2345538c0c6f582fCB022739c4A194Ebb'),
                     lower('0xe50fA9b3c56FfB159cB0FCA61F5c9D750e8128c8'),
                     lower('0x513c7E3a9c69cA3e22550eF58AC1C0088e918FFf'))
...
gov_restakes AS (
    ...
    FROM {{ source('optimism_gold_core','ez_decoded_event_logs') }}
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address IN (lower('0x6806411765Af15Bddd26f8f544A34cC40cb9838B'),
                     lower('0xadDb6A0412DE1BA0F936DCaeb8Aaa24578dcF3B2'),
                     lower('0xE405de8F52ba7559f9df3C368500B6E6ae6Cee49'),
                     lower('0xc45A479877e1e9Dfe9FcD4056c699575a1045dAA'),
                     lower('0x1F32b1c2345538c0c6f582fCB022739c4A194Ebb'),
    ...
    AND origin_from_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
    AND origin_from_address NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
...
lp_adds AS (
    SELECT 
        ...
        COUNT(*) AS n_lp_adds
    FROM {{ source('optimism_gold_core','ez_token_transfers') }}
    WHERE TO_ADDRESS IN (SELECT contract_address FROM optimism.defi.ez_dex_swaps WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND TX_HASH NOT IN (SELECT TX_HASH FROM optimism.defi.ez_dex_swaps WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_labels')}})
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('optimism_gold_core','dim_contracts') }})
    GROUP BY FROM_ADDRESS
),
...
tx_hash
FROM
{{ source('optimism_gold_core','ez_token_transfers') }}
WHERE
to_address IN (SELECT pool_address FROM optimism.defi.dim_dex_liquidity_pools)
AND
block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
GROUP BY tx_hash
),
...
other_defi AS (
    ...
    FROM {{ source('optimism_gold_core','ez_decoded_event_logs') }}
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address NOT IN (lower('0x6806411765Af15Bddd26f8f544A34cC40cb9838B'),
                     lower('0xadDb6A0412DE1BA0F936DCaeb8Aaa24578dcF3B2'),
                     lower('0xE405de8F52ba7559f9df3C368500B6E6ae6Cee49'),
                     lower('0xc45A479877e1e9Dfe9FcD4056c699575a1045dAA'),
                     lower('0x1F32b1c2345538c0c6f582fCB022739c4A194Ebb'),
...
scores AS (
    SELECT
        user_address,
        (CASE WHEN n_days_active > 7 THEN 1 ELSE 0 END
         + CASE WHEN n_complex_txn > 0 THEN 1 ELSE 0 END
         + CASE WHEN n_contracts > 4 THEN 1 ELSE 0 END) AS activity_score,
        (CASE WHEN n_bridge_in > 5 THEN 1 ELSE 0 END
        ...
        (CASE WHEN n_other_defi > 0 THEN 1 ELSE 0 END
         + CASE WHEN n_swap_tx > 7 THEN 1 ELSE 0 END
         + CASE WHEN n_lp_adds > 4 THEN 1 ELSE 0 END) AS defi_score,
        ...
        (CASE WHEN n_stake_tx > 0 THEN 1 ELSE 0 END
         + CASE WHEN n_validators > 4 THEN 1 ELSE 0 END
         + CASE WHEN n_restakes > 2 THEN 1 ELSE 0 END) AS gov_score
    FROM final_output
),

total_scores AS (
    SELECT
        {{ dbt_utils.generate_surrogate_key(['user_address', "'optimism'", "'" ~ current_date_var ~ "'"]) }} AS id,        
