{{ config (
    ...
    full_refresh = false,
    tags = ['gold', 'onchain_scores', 'polygon_scores'],
    version = 1
) }}
...
WITH cex_addresses AS (
    ...
    FROM {{ source('polygon_gold_core','dim_labels')}}
...
activity AS (
    ...
    FROM {{ source('polygon_gold_core','fact_transactions') }}
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }} )
    AND from_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }} )
...
from_bridge_daily AS (
    ...
    FROM {{ source('polygon_gold_defi','ez_bridge_activity') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND receiver NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
    AND receiver NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
...
from_cex_daily AS (
    ...
    FROM {{ source('polygon_gold_core','ez_native_transfers') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND from_address IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
    ...
    SELECT 
        ...
        DATE(block_timestamp) AS activity_day,
        COUNT(*) AS n_cex_withdrawals
    FROM {{ source('polygon_gold_core','ez_token_transfers') }} 
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND FROM_ADDRESS IN (SELECT ADDRESS FROM cex_addresses)
    AND to_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
    AND to_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
...
nft_buys AS (
    ...
    FROM {{ source('polygon_gold_nft','ez_nft_sales') }} 
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND buyer_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
    AND buyer_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
...
from {{ source('polygon_gold_core','ez_decoded_event_logs') }} 
where topics[0] in (
...
FROM rawmints
WHERE
decoded_log:to NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
AND 
decoded_log:to NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
group by user_address
),
-- governance (just liquid staking on polygon)
gov_mints AS (
    SELECT
        to_address AS user_address,
    ...
    FROM {{ source('polygon_gold_core','ez_token_transfers') }}
    WHERE 
        contract_address IN (lower('0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4'),
                     lower('0xa3Fa99A148fA48D14Ed51d610c367C61876997F1'),
                     lower('0x2acD702F7D35d3D2915663d7f7CbDF2863Ec6E79'),
                     lower('0xfa68FB4628DFF1028CFEc22b4162FCcd0d45efb6'),
                     lower('0x0E9b89007eEE9c958c0EDA24eF70723C2C93dD58'))
...
gov_restakes AS (
    ...
    FROM {{ source('polygon_gold_core','ez_decoded_event_logs') }}
    WHERE BLOCK_TIMESTAMP >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address IN (lower('0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4'),
                     lower('0xa3Fa99A148fA48D14Ed51d610c367C61876997F1'),
                     lower('0x2acD702F7D35d3D2915663d7f7CbDF2863Ec6E79'),
                     lower('0xfa68FB4628DFF1028CFEc22b4162FCcd0d45efb6'),
                     lower('0x0E9b89007eEE9c958c0EDA24eF70723C2C93dD58'))
    AND event_name IN ('Deposit', 'Submitted', 'DepositedFromStaking')
    AND origin_from_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
    AND origin_from_address NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
...
lp_adds AS (
    SELECT 
        ...
        COUNT(*) AS n_lp_adds
    FROM {{ source('polygon_gold_core','ez_token_transfers') }}
    WHERE TO_ADDRESS IN (SELECT contract_address FROM polygon.defi.ez_dex_swaps WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND TX_HASH NOT IN (SELECT TX_HASH FROM polygon.defi.ez_dex_swaps WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90)
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_labels') }})
    AND FROM_ADDRESS NOT IN (SELECT address FROM {{ source('polygon_gold_core','dim_contracts') }})
    GROUP BY FROM_ADDRESS
),
...
tx_hash
FROM
{{ source('polygon_gold_core','ez_token_transfers') }}
WHERE
to_address IN (SELECT pool_address FROM polygon.defi.dim_dex_liquidity_pools)
AND
block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
GROUP BY tx_hash
),
...
other_defi AS (
    ...
    FROM {{ source('polygon_gold_core','ez_decoded_event_logs') }}
    WHERE block_timestamp >= CAST( '{{ current_date_var }}' AS DATE) - 90
    AND contract_address NOT IN (lower('0x3A58a54C066FdC0f2D55FC9C89F0415C92eBf3C4'),
                     lower('0xa3Fa99A148fA48D14Ed51d610c367C61876997F1'),
                     lower('0x2acD702F7D35d3D2915663d7f7CbDF2863Ec6E79'),
                     lower('0xfa68FB4628DFF1028CFEc22b4162FCcd0d45efb6'),
                     lower('0x0E9b89007eEE9c958c0EDA24eF70723C2C93dD58'))
...
scores AS (
    SELECT
        user_address,
        (CASE WHEN n_days_active > 4 THEN 1 ELSE 0 END
        ...
        (CASE WHEN n_bridge_in > 2 THEN 1 ELSE 0 END
         + CASE WHEN n_cex_withdrawals > 1 THEN 1 ELSE 0 END
        ...
        (CASE WHEN n_nft_mints > 0 THEN 1 ELSE 0 END
         ...
         + CASE WHEN n_nft_trades > 2 THEN 1 ELSE 0 END) AS nfts_score,
...
total_scores AS (
    SELECT
        {{ dbt_utils.generate_surrogate_key(['user_address', "'polygon'", "'" ~ current_date_var ~ "'"]) }} AS id,        
