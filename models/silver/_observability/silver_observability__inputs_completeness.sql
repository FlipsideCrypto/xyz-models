...
inputs_per_block_expected AS (
    SELECT
        block_number,
        SUM(ARRAY_SIZE(inputs)) AS inputs
    FROM
...
comparison AS (
    SELECT
        id.block_number,
        A.inputs AS actual_input_count,
        e.inputs AS expected_input_count
    FROM
        block_range id
        LEFT JOIN inputs_per_block_actual A USING (block_number)
        LEFT JOIN inputs_per_block_expected e USING (block_number)
),
impacted_blocks AS (
    SELECT
