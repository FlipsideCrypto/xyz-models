...
outputs_per_block_expected AS (
    SELECT
        block_number,
        SUM(ARRAY_SIZE(outputs)) AS outputs
    FROM
...
comparison AS (
    SELECT
        id.block_number,
        A.outputs AS actual_output_count,
        e.outputs AS expected_output_count
    FROM
        block_range id
        LEFT JOIN outputs_per_block_actual A USING (block_number)
        LEFT JOIN outputs_per_block_expected e USING (block_number)
),
impacted_blocks AS (
    SELECT
